...
public class AuCalcolaIndicatoriDaoImpl implements AuCalcolaIndicatoriDao {
	...
	public void getDatiCampagnaVarCompDto(long idCampagna,
		...
		String queryStr = " Select t1.ID_FASE, t1.ID_M_NONCONF, t1.ID_M_VARCOMP, t1.DATA_INIZIO, t1.DATA_FINE, NUM_VC, NUM_NC, SOGLIA, (cast((NUM_VC) as decimal (7,2))/cast((NUM_NC) as decimal (7,2))) AS PERC_SU_PS, (cast((NUM_VC) as decimal (7,2))/cast((NUM_NC) as decimal (7,2))*SOGLIA) AS PERC_PESATA  from "
							...
							+ " left join  "
							+ " (Select svc1.ID_M_NONCONF, svc1.ID_M_VARCOMP, mnc.ID_FASE, cam.DATA_INIZIO, cam.DATA_FINE, soglia, sum (svc1.num) as NUM_VC "
							+ " FROM "
							...
							+ " cam.id_campagna = sess.id_campagna and "
							+ " ID_TPL_ISNC = mvc.PESO_VC and "
							+ " sess.ID_CAMPAGNA = " + idCampagna + " and "
							+ " ausess.STATO_ESAME_SESSIONE = 'C' "
							+ " group by svc1.ID_M_NONCONF, svc1.ID_M_VARCOMP, mnc.ID_FASE, cam.DATA_INIZIO, cam.DATA_FINE, soglia) as t1 on t2.ID_M_NONCONF = t1.ID_M_NONCONF group by svc1.ID_M_NONCONF, svc1.ID_M_VARCOMP, mnc.ID_FASE, svc1.DATA_INIZIO, svc1.DATA_FINE, soglia) as t1 on t2.ID_M_NONCONF = t1.ID_M_NONCONF  "
